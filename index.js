...
const { decodeData, encodeData, marshallFileUpdate, log, error } = require('./utils');

dotenv.config();

async function updateProviders(data) {
	const providers = 'https://api.github.com/repos/ShenaniganDApp/ParticleAccelerator/contents/data/providers.json';
	...
	try {
		fetch(providers, {
          ...
          console.log('provider.pair.totalSupply : ', provider.pair.totalSupply );					const userExists = decodedContent.findIndex(
						(oldProvider) => oldProvider.user.address === provider.user.address
					);
					if (userExists !== -1) {
						liquidityProviders.push({
							...decodedContent[userExists],
							liquidityTokenBalance: provider.liquidityTokenBalance,
              pair: { totalSupply: provider.pair.totalSupply }
						});
					} else {
